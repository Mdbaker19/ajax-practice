$(document).ready(function (){
    ...
    function renderOption(){
        let html = `<option value="" disabled selected>Select Your country....</option>`
        for(let i = 0; i < countryNames.length; i++){
            html += `<option value=${countryKeys[i]}>${countryNames[i]}</option>`;
        }
        return html;
    }


    document.querySelector("#countryList").insertAdjacentHTML("beforeend", renderOption())
    ...
    $("#submit").on("click", function (){
        ...
        let countryInput = $("#countryList").val();
        getAge(name, countryInput).then(data => {
            $("#output").html(renderOutput(data));
        });
    });
